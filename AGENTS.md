...
- `.claude-plugin/plugin.json`: portable package metadata; do not touch active Claude config.
...
- `shared/assets/templates/`: non-overwriting project templates.
...
- Keep portable instructions in `SKILL.md`. Codex-only presentation belongs in
  `agents/openai.yaml`, whose default prompt must name the corresponding `$skill-name`.
